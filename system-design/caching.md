...
14. What trade-off does Redis vs in-process cache represent?
...
14. The trade-off in Redis vs in-process cache is about choosing the construct whose constraints match the problem. Good candidates explain both sides instead of declaring one universally better.
...
|---|---|---|
| Redis | Use when it makes intent direct and maintainable. | Usually simpler, but may be less flexible. |
| in-process cache | Use when constraints require it. | Often more powerful, but can add complexity. |

## Coding Challenge
Write a function `explain_tradeoff(choice: str) -> str` that returns a concise recommendation for when to use one side of `Redis vs in-process cache`.
